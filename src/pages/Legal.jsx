...
const PRIVACY = {
  ...
  sections: [
    ...
    { h: 'How your data is stored', b: 'Your invoices, products, stores, notes, payment records and signatures are stored in your account in Supabase and protected by per-user Row Level Security, so no other account can read them. A copy is also cached locally on your device so the app works offline.' },
    { h: 'What other users can see', b: 'Marketplace is a public noticeboard inside the app, and it is the one exception to the above. Anything you publish there — a driver listing or a store request — is readable by every signed-in Keiro user, not only the ones you are connected to. That includes the business name and phone number you publish with it, the product, the price or quantity, your notes, and, if you allowed location access, the coordinates recorded when you created it, which is what powers near-me sorting. Nothing you keep outside Marketplace is exposed this way. Treat a Marketplace post as public: do not put anything in one you would not show a stranger.' },
